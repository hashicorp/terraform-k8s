function tag_release {
   # Arguments:
   #   $1 - Path to top level terraform source
...
function package_binaries {
   ...
   do
      ...
      local dest="${ddir}/${TERRAFORM_PKG_NAME}_${vers}_${os_arch}.zip"
      status "Compressing ${os_arch} directory into ${dest}"
      pushd "${platform}" > /dev/null
      zip "${ddir}/${TERRAFORM_PKG_NAME}_${vers}_${os_arch}.zip" ./*
      ret=$?
      popd > /dev/null
...
function package_release_one {
   # Arguments:
   #   $1 - Path to the top level Terraform source
...
function shasum_release {
   ...
   local hfile="${TERRAFORM_PKG_NAME}_${vers}_SHA256SUMS"
...
function check_release_one {
   ...
   expected_files+=("${TERRAFORM_PKG_NAME}_${2}_SHA256SUMS")
   echo "check sig: $3"
   if is_set "$3"
   then
      expected_files+=("${TERRAFORM_PKG_NAME}_${2}_SHA256SUMS.sig")
   fi

   expected_files+=("${TERRAFORM_PKG_NAME}_${2}_darwin_386.zip")
   expected_files+=("${TERRAFORM_PKG_NAME}_${2}_darwin_amd64.zip")
   expected_files+=("${TERRAFORM_PKG_NAME}_${2}_freebsd_386.zip")
   expected_files+=("${TERRAFORM_PKG_NAME}_${2}_freebsd_amd64.zip")
   expected_files+=("${TERRAFORM_PKG_NAME}_${2}_freebsd_arm.zip")
   expected_files+=("${TERRAFORM_PKG_NAME}_${2}_linux_386.zip")
   expected_files+=("${TERRAFORM_PKG_NAME}_${2}_linux_amd64.zip")
   expected_files+=("${TERRAFORM_PKG_NAME}_${2}_linux_arm.zip")
   expected_files+=("${TERRAFORM_PKG_NAME}_${2}_linux_arm64.zip")
   expected_files+=("${TERRAFORM_PKG_NAME}_${2}_solaris_amd64.zip")
   expected_files+=("${TERRAFORM_PKG_NAME}_${2}_windows_386.zip")
   expected_files+=("${TERRAFORM_PKG_NAME}_${2}_windows_amd64.zip")
   ...
   if test $ret -eq 0
   then
      if ! shasum -c -s "${TERRAFORM_PKG_NAME}_${2}_SHA256SUMS"
      then
         err "ERROR: Failed SHA-256 hash verification"
         shasum -c "${TERRAFORM_PKG_NAME}_${2}_SHA256SUMS"
         ret=1
      fi
   fi
   ...
   then
      if ! gpg --verify "${TERRAFORM_PKG_NAME}_${2}_SHA256SUMS.sig" "${TERRAFORM_PKG_NAME}_${2}_SHA256SUMS" > /dev/null 2>&1
      then
...
function build_terraform_release {
   build_terraform "$1" "" "$2"
}
...
function build_release {
   ...
   then
      err "ERROR: build_release requires 4 arguments to be specified: <path to terraform source> <tag release bool?> <build binaries bool?> <shasum 256 bool?>"
      return 1
   fi
   ...
   then
      ...
      build_terraform_release "${sdir}" "${GO_BUILD_TAG}"
      if test $? -ne 0
      then
