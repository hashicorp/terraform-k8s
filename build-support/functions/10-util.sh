...
function update_git_env {
   ...
   export GIT_IMPORT=github.com/hashicorp/terraform-k8s/version
...
function shasum_directory {
 ...
 function ui_version {
   ...
   local ui_version=$(sed -n ${SED_EXT} -e 's/.*TERRAFORM_VERSION%22%3A%22([^%]*)%22%2C%22.*/\1/p' < "$1") || return 1
   echo "$ui_version"
   return 0
 }
 function ui_logo_type {
   ...
   # and doesn't necessarily correspond to the binary type of terraform
