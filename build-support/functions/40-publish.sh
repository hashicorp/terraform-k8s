   ...
   do
      case "${answer}" in
         ...
         [nN]* )
            ...
            ;;
         * )
            read -p "Is this Git Remote correct to push ${TERRAFORM_PKG_NAME} to? [y/n]: " answer
            ;;
      esac
   done
}
...
function extract_terraform_local {
   # Arguments:
   ...
   local zfile="${1}/${TERRAFORM_PKG_NAME}_${2}_$(go env GOOS)_$(go env GOARCH).zip"
   ...
   local ret=0
   local tfile="$(mktemp) -t "${TERRAFORM_PKG_NAME}_")"

   unzip -p "${zfile}" "terraform-k8s" > "${tfile}"
   if test $? -eq 0
   then
      chmod +x "${tfile}"
      echo "${tfile}"
      ...
      err "ERROR: Failed to extract terraform binary from the zip file"
      return 1
   fi
}

function confirm_terraform_version {
   # Arguments:
   #   $1 - terraform exe to use
   #
   # Returns:
   #   0 - success
   #   * - error
   local terraform_exe="$1"

   if ! test -x "${terraform_exe}"
   then
      err "ERROR: '${terraform_exe} is not an executable"
      return 1
   fi
   ...
   do
      case "${answer}" in
         ...
         [nN]* )
            ...
            ;;
         * )
            read -p "Is this Terraform version correct? [y/n]: " answer
            ;;
      esac
   done
}
...
function verify_release_build {
   ...
   status_stage "==> Extracting Terraform version for local system"
   local terraform_exe=$(extract_terraform "${sdir}/pkg/dist" "${vers}") || return 1
   # make sure to remove the temp file
   trap "rm '${terraform_exe}'" EXIT

   status_stage "==> Confirming Terraform Version"
   confirm_terraform_version "${terraform_exe}" || return 1
}

function publish_release {
   # Arguments:
   #   $1 - Path to top level Terraform source that contains the built release
