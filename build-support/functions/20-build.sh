...
function build_terraform_post {
   ...
   if ! test -d "$1"
   then
      err "ERROR: '$1' is not a directory. build_terraform_post must be called with the path to the top level source as the first argument'"
      return 1
   fi
...
function build_terraform {
   ...
   #   If the TERRAFORM_DEV environment var is truthy only the local platform/architecture is built.
   ...
   if ! test -d "$1"
   then
      err "ERROR: '$1' is not a directory. build_terraform must be called with the path to the top level source as the first argument'"
      return 1
   fi
   ...
   local container_id=$(docker create -v "${MAIN_GOPATH}/pkg/mod:/go/pkg/mod:ro" -it -e CGO_ENABLED=0 ${image_name} gox -os="${XC_OS}" -arch="${XC_ARCH}" -osarch="!darwin/arm !darwin/arm64" -ldflags "${GOLDFLAGS}" -output "pkg/bin/${extra_dir}{{.OS}}_{{.Arch}}/terraform-k8s" -tags="${GOTAGS}")
   ret=$?
   ...
   then
      ...
      if test $ret -eq 0
      then
         build_terraform_post "${sdir}" "${extra_dir_name}"
         ret=$?
      else
...
function build_terraform_local {
   # Arguments:
   #   $1 - Path to the top level Terraform source
   ...
   status_stage "==> Building Terraform - OSes: ${build_os}, Architectures: ${build_arch}"
   ...
   then
      ...
      CGO_ENABLED=0 gox \
         ...
         -output "pkg.bin.new/${extra_dir}{{.OS}}_{{.Arch}}/terraform-k8s" \
         -tags="${GOTAGS}" \
         .

      if test $? -ne 0
      then
         err "ERROR: Failed to build Terraform"
         rm -r pkg.bin.new
         return 1
      fi
   else
      ...
      do
         ...
         do
            ...
            CGO_ENABLED=0 GOOS=${os} GOARCH=${arch} go install -ldflags "${GOLDFLAGS}" -tags "${GOTAGS}" && cp "${MAIN_GOPATH}/bin/${GOBIN_EXTRA}"/terraform-k8s "${outdir}/terraform-k8s"
            if test $? -ne 0
            then
               err "ERROR: Failed to build Terraform for ${osarch}"
               rm -r pkg.bin.new
               return 1
            fi
         done
   ...
   if test $? -ne 0
   then
      err "ERROR: Failed postprocessing Terraform binaries"
      return 1
   fi
   return 0
}
